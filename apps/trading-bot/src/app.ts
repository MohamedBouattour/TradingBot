...
async function main() {
  ...
  LogService.logStructured('INFO', 'SYSTEM', "Trading Bot starting up...", startupData);
  ...
  try {
    ...
    if (process?.env?.["MODE"] !== "DEBUG" && timeToCloseCurrentCandle > 0) {
      ...
      const waitData = {
        timeToWait: (timeToCloseCurrentCandle / (1000 * 60)).toFixed(2) + " minutes",
        timeToWaitMs: timeToCloseCurrentCandle,
        nextCandleTime: new Date(candlesticks[candlesticks.length - 1].closeTime).toISOString(),
        timestamp: new Date().toISOString()
      };
      
      LogService.logStructured('INFO', 'SYSTEM', "Waiting for next candle to open", waitData);
    ...
    LogService.logStructured('INFO', 'SYSTEM', "Trading Bot started successfully", startupCompleteData);
    ...
    while (true) {
      try {
        ...
        LogService.logError(`Trading loop error (attempt ${retryCount})`, errorData);
        LogService.logStructured('WARN', 'SYSTEM', `Retrying in ${backoffDelay / 1000} seconds...`, { retryCount, backoffDelay });
...
async function rebalancePorfolio() {
  try {
    ...
    const successCount = rebalanceResults.filter(r => r.status === 'SUCCESS').length;
    const errorCount = rebalanceResults.filter(r => r.status === 'ERROR').length;

...
export class ApiClientService {
  private static readonly API_BASE_URL = process.env?.["BOT_API_URL"] || 'http://localhost:3002/api';
  ...
  static async sendTradingDecision(decisionData: TradingDecisionData): Promise<boolean> {
    const result = await this.makeRequest('/trading/decisions', 'POST', decisionData);
    return result !== null;
  }
  ...
  static async sendRebalanceResult(rebalanceResult: RebalanceResult): Promise<boolean> {
    const result = await this.makeRequest('/trading/rebalance', 'POST', rebalanceResult);
    return result !== null;
  }

  // Portfolio Methods
  static async updatePortfolioValue(asset: string, valueInBaseCurrency: number): Promise<boolean> {
  ...
  static async batchUpdatePortfolioValues(updates: { asset: string; valueInBaseCurrency: number }[]): Promise<void> {

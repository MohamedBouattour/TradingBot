...
export class PortfolioController {
  ...
  async getStats() {
    ...
    const totalValueInBaseCurrency = await this.portfolioService.getTotalValueInBaseCurrency();
    ...
    return {
      ...
      assets: portfolio.map(item => ({
        ...
        deviation: item.valueInBaseCurrency ?
          ((item.valueInBaseCurrency - item.value) / item.value * 100) : 0,
  ...
  async findOne(@Param('asset') asset: string): Promise<PortfolioItemDto> {
    ...
    if (!item) {
      throw new NotFoundException(`Portfolio item with asset ${asset} not found`);
    }
    return item;
  }

  @Post()
  async create(@Body() createPortfolioItemDto: CreatePortfolioItemDto): Promise<PortfolioItemDto> {
  ...
  async update(
    ...
    const updatedItem = await this.portfolioService.update(asset, updatePortfolioItemDto);
    if (!updatedItem) {
      throw new NotFoundException(`Portfolio item with asset ${asset} not found`);
    }
    return updatedItem;
  ...
  async updateValue(
    ...
    const updatedItem = await this.portfolioService.updateValueInBaseCurrency(asset, valueInBaseCurrency);
    if (!updatedItem) {
      throw new NotFoundException(`Portfolio item with asset ${asset} not found`);
    }
    return updatedItem;
